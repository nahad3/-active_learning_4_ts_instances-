    ...
    for k in range(0,no_loops):
        ...
        for ep in range(0, args.no_epochs):
            ...
            for data in val_dataload:
                ...
                if ep %100 == 0:
                    ...
                    cm = confusion_matrix(y_true, y_predicted.cpu().numpy())
        ...
        for data in train_dataloader:
            ...
            if args.query_type == 1:
                ...
                top_10_centroids = [val[0] for val in sorted_freq[0:int(args.no_queries/no_batches)]]
                top_10_kmeans = kmeans.cluster_centers_[top_10_centroids, :]
                # pool_sample_add =  [val[0] for val in sorted_freq[0:3]]
                nearest_indices = [find_nearest(x_src_feats_reshaped, top_10_kmeans[value, :]) for value in range(0, int(args.no_queries/no_batches))]
                top_k_ent_indices = np.asarray(nearest_indices )
            # get new queries for batch

            pool_sample_add[btch, :] = top_k_ent_indices[0:int(args.no_queries/no_batches)]
            btch = btch + 1
