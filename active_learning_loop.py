...
def train_active_loop(model_feats, model_clfr, train_dataloader, args, device, wanb_con, val_dataload,
    ...
    for k in range(0, numb_rounds):
            ...
            for ep in range(0, args.no_epochs):
                ...
                if val_each_epoch:
                    ...
                    for data in val_dataload:
                        ...
                        rand_samples = np.random.choice(np.arange(0, no_labels_p_batch),
                                                        size=int(queries_per_round / 1))
        ...
        for data in val_dataload:
            ...
            if numb_rounds > 1:
                active_log_dict_f1 = {
                    f"Val F1 score/{args.dict_type[args.query_type]}/  for run number {args.run_no} ": f1_score,
        ...
        if numb_rounds > 1:
            ...
            for data in train_dataloader:
                ...
                # divide the data into labeled and unlabeled sets
                ...
                src_lbl_clfr_reshaped_u = src_lbl_clfr_reshaped[ind_u]

                if args.query_type == 1:
                    probs = torch.nn.functional.softmax(src_lbl_clfr_reshaped_u, dim=1)
                    ...
                    top_k_ent_indices_pseudo = torch.topk(entropy, int(queries_per_round / no_batches))[1].cpu().numpy()

                elif args.query_type == 0:
                    top_k_ent_indices_pseudo = np.random.choice(np.arange(0,
                                                                          src_lbl_clfr_reshaped_u.shape[0]),
                    ...
                    x_src_feats_reshaped_u = embeddings_u.detach().cpu().numpy()
                    x_src_feats_filtr = x_src_feats_reshaped_u[entrop_filt, :]
                    ...
                    "Ensure nearest indices are not from existing pool values - not required since now we are only considering unlabeled samples"
                    #                    x_src_feats_reshaped[pool[btch,:].astype(int),:] = 1e5
                    nearest_indices = [find_nearest(x_src_feats_reshaped_u, top_10_kmeans[value, :]) for value in
                                       range(0, int(queries_per_round / no_batches))]
                    top_k_ent_indices_pseudo = np.asarray(nearest_indices)
                    ...
                    x_src_feats_reshaped_u = embeddings_u.detach().cpu().numpy()
                    x_src_feats_filtr = x_src_feats_reshaped_u[marg_confd_filt, :]
                    ...
                    num_clusters = 2 * n_samples

                    #   kmeans clustering
                    cluster_ids, cluster_centers = kmeans_fit(X=embeddings_u, num_clusters=num_clusters,
                    ...
                    for k in range(num_clusters):
                        ...
                        temp = samples_u[true_ind][:, 1]
                        pseudo_ind = torch.argmax(samples_u[true_ind][:, 1])
                        topk_true_ind = true_ind[0][pseudo_ind]
                        cand_samples.append(topk_true_ind)

                    temp3 = samples_u[cand_samples, 1]
                    _, pseudo_ind = torch.topk(samples_u[cand_samples, 1], n_samples, largest=True)
                    pseudo_ind_cpu = pseudo_ind.cpu().detach().numpy()
                    ind_list = np.asarray(pseudo_ind_cpu)
                    next_samples = torch.tensor(cand_samples)[ind_list]
                    next_samples = next_samples.cpu().detach().numpy()
                    top_k_ent_indices_pseudo = np.ndarray.flatten(np.squeeze(next_samples))
                ...
                top_k_ent_indices = np.asarray(ind_u)[top_k_ent_indices_pseudo]
            ...
            if visualize == 1 and args.run_no == 0 and (pool.reshape(-1, ).shape[0]) % 40 == 0:
                ...
                if args.query_type == 0:
                    top_10_kmeans = None
                fig = plot_ts_reps(model_feats=model_feats, model_clfr=model_clfr, x=x.numpy(), y=y.numpy(),
                                   device=device, clusters=top_10_kmeans,
                                   window=-1, title='str(i)')
                wanb_con.log({
                                 f"RepresetSubplots/Val/{args.dict_type[args.query_type]}/ after pool of size  {pool.reshape(-1, ).shape[0]}": fig})
                print("here")
